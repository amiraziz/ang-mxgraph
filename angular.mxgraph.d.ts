...
declare let mxLanguages: string[];
...
declare let mxResourceExtension: string;
...
declare class mxShape {
  ...
  /**
   * Implicit letiable declarations
   */
  strokewidth: number;
  rotation: number;
  opacity: number;
...
declare let mxUtils: {
   ...
   */
  clone(obj: any, transients?: any, shallow?: any): any,
   ...
   */
  makeDraggable(element: any, graphF: any, funct: any, dragElement?: any, dx?: any, dy?: any, autoscroll?: any,
                scalePreview?: any, highlightDropTargets?: any, getDropTarget?: any): mxDragSource,
  /**
   ...
   */
  getPrettyXml(node: any, tab?: any, indent?: any): any,
  /**
   ...
   */
  setOpacity(node: any, value: number): any,
  /**
   ...
   */
  isAncestorNode(ancestor: any, child: any),
  /**
   * Function: bind
   *
...
declare class mxToolbar extends mxEventSource {
   ...
   * be selected at a time. The currently tabControl item is the default item
...
declare class mxPopupMenu extends mxEventSource {
   ...
   * time. This adds letious DOM nodes and a <submenuImage> to the parent.
   *
   * Parameters:
   *
 ...
 * encoded image data, eg. "cardData:image/gif,XYZ", where XYZ is the base64 encoded
 * image data.
 *
 ...
 * turned into a data URI if the returned value has a short cardData URI format
 * as specified above.
 *
...
declare class mxSvgCanvas2D extends mxAbstractCanvas2D {
  ...
  foEnabled: boolean;
  /**
   * Specifies the fallback text for unsupported foreignObjects in declareed
   * documents. Default is '[Object]'. If this is set to null then no fallback
   * text is added to the declareed document.
   */
  foAltText: string;
  /**
...
declare class mxGraphModel extends mxEventSource {
  ...
  valueForCellChanged(cell: any, value: any): any;
 ...
 * follows. The url letiable is the URL of the Java servlet, PHP page or HTTP
 ...
 * mxUtils.load can be used as follows. The url letiable is the URL of the Java
...
declare class mxGraph extends mxEventSource {
   ...
   * turns short data URIs as defined in mxImageBundle to cardData URIs as
   ...
   * returns true the cell behind the selected cell will be tabControl. This
  ...
  /**
   * Returns the cells which may be declareed in the given array of cells.
   */
  ...
  /**
   * Returns true if the given cell may be declareed to the clipboard. This
   * implementation returns <declareEnabled> for all cells.
   *
   * cell - <mxCell> that represents the cell to be declareed.
   */
...
declare class mxOutline {
  ...
  addButtons(okFunct: any, cancelFunct: any): any;
 ...
 *
 * To save diagrams in XML on a server, you can set the <urlPost> letiable.
 * This letiable will be used in <getUrlPost> to construct a URL for the post
 ...
 *
 * Here are some examples of processing a post request in letious languages.
 *
...
declare class mxEditor extends mxEventSource {
   ...
   * declareImage - Shows the graph as a bitmap image using <getUrlImage>.
 ...
 * The letiables in the optional exclude array are ignored by
 * the codec. letiables in the optional idrefs array are
 * turned into references in the XML. The optional mapping
 * may be used to map from letiable names to XML attributes.
 * The argument is created as follows:
 *
 * (code)
 * let mapping = new Object();
 * mapping['letiableName'] = 'attribute-name';
 * (end)
 *
 * Parameters:
 *
...
declare class mxObjectCodec {
   ...
   *
   * This implementation encodes all letiables of an
   * object according to the following rules:
   *
   * - If the letiable name is in <exclude> then it is ignored.
   * - If the letiable name is in <idrefs> then <mxCodec.getId>
   * is used to replace the object with its ID.
   * - The letiable name is mapped using <mapping>.
   * - If obj is an array and the letiable name is numeric
   * (ie. an index) then it is not encoded.
   * - If the value is an object, then the codec is used to
   * create a child node with the letiable name encoded into
   * the "as" attribute.
   ...
   *
   * If no ID exists for a letiable in <idrefs> or if an object
   ...
   *
   * - If the letiable name is in <exclude> or if the attribute name is "id"
   * or "as" then it is ignored.
   * - If the letiable name is in <idrefs> then <mxCodec.getObject> is used
   * to replace the reference with an object.
   * - The letiable name is mapped using a reverse <mapping>.
   * - If the value has a child node, then the codec is used to create a
   * child object with the letiable name taken from the "as" attribute.
   * - If the object is an array and the letiable name is empty then the
   * value or child object is appended to the array.
   * - If an add child has no value or the object is not an array then
   * the child text content is evaluated using <mxUtils.eval>.
   *
   * For add nodes where the object is not an array and the letiable name
...
declare let mxHierarchicalEdgeStyle: {
  ORTHOGONAL: number;
  POLYLINE: number;
  STRAIGHT: number;
  CURVE: number;
...
declare class mxGraphAbstractHierarchyCell {
   ...
   */
  getGeneralPurposeletiable(layer: any): any;

  /**
   * Set the value of temp for the specified layer
   */
  setGeneralPurposeletiable(layer: any, value: any): any;
...
declare class mxGraphHierarchyNode extends mxGraphAbstractHierarchyCell {
   ...
   * Set the value of temp for the specified layer
   */
  setGeneralPurposeletiable(layer: any, value: any): void;
